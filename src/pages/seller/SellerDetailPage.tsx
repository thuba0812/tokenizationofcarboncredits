...
export default function SellerDetailPage() {
  ...
  const project = useMemo(() => {
    ...
    return matchedCredits.reduce((acc, credit, index) => {
      if (index === 0) {
        return {
          ...credit.project,
  ...
  if (!project) {
    return (
      <div className="p-10 text-center text-gray-400">
        Không tìm thấy tài sản của dự án này trong ví hiện tại.
      </div>
    );
  }
  ...
  const getRowErrors = (token: (typeof mintedTokens)[number]): RowErrors => {
    ...
    const hasExistingListing = Boolean(
      token.currentListingId || token.onchainListingId || token.listedAmount,
    );
    ...
    try {
      ...
      if (isContractConfigured()) {
          ...
          steps.push({
            ...
            run: async () => {
              const result = await contractService.cancelListingDetailed(
                token.onchainListingId as number,
              );
        ...
        if (!result.success) {
          setSaveError(
            txState.error ||
              "Không thể cập nhật lệnh đăng bán trên blockchain.",
          );
          return;
        }
      ...
      if (!success) {
        setSaveError(
          txHash
            ? "Blockchain đã cập nhật, nhưng lưu Supabase thất bại."
            : "Không thể lưu thay đổi vào Supabase.",
        );
        return;
      }
      ...
      setSaveError(
        error instanceof Error ? error.message : "Cập nhật đăng bán thất bại.",
      );
    } finally {
      setSavingVintageId(null);
    }
        ...
        <div className="grid grid-cols-1 gap-8 lg:grid-cols-3">
          <div className="lg:col-span-2">
            <div className="rounded border border-gray-200 p-6">
              <h2 className="mb-5 font-heading text-lg font-bold tracking-widest text-gray-700">
                THÔNG TIN DỰ ÁN
              </h2>
              ...
              <table className="w-full">
                <thead>
                  <tr className="border-b border-gray-200 bg-gray-50">
                    <th className="px-5 py-3 text-left font-heading text-xs font-bold tracking-widest text-gray-400">
                      NĂM
                    </th>
                    <th className="px-5 py-3 text-left font-heading text-xs font-bold tracking-widest text-gray-400">
                      TOKEN ID
                    </th>
                    <th className="px-5 py-3 text-center font-heading text-xs font-bold tracking-widest text-gray-400">
                      HIỆN CÓ
                    </th>
                    <th className="px-5 py-3 text-center font-heading text-xs font-bold tracking-widest text-gray-400">
                      ĐANG BÁN
                ...
                <tbody>
                  {mintedTokens.map((token) => {
                    ...
                    return (
                      <tr
                        key={token.vintageId}
                        className="border-b border-gray-50 align-top last:border-0 hover:bg-gray-50"
                      >
                            ...
                            {isEditing ? (
                              <>
                                <button
                                  ...
                                  disabled={
                                    Boolean(errors.quantity || errors.price) ||
                                    isSaving ||
                                    txState.isLoading
                                  }
            ...
            {editingVintageId ? (
              <div className="mt-4 rounded border border-amber-200 bg-amber-50 px-4 py-3 text-sm text-amber-800">
                Nếu đây là lần đầu dùng ví này để đăng bán, MetaMask có thể yêu
                cầu xác nhận 2 giao dịch: cấp quyền cho Marketplace và cập nhật
                lệnh đăng bán.
              </div>
            ) : null}
...
function RepField({
  ...
  return (
    <div className="mb-3">
      <div className="mb-0.5 font-heading text-xs font-bold tracking-widest text-gray-400">
        {label}
      </div>
      {link ? (
        <a
